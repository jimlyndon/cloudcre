...
namespace Cloudcre.Report.Summary
{
    public class SummaryReport
    {
        ...
        public void CreatePackage(string filePath, IEnumerable<PropertyViewModel> viewModels)
        {
        ...
        public void CreatePackage(MemoryStream mStream, IEnumerable<PropertyViewModel> viewModels)
        {
        ...
        private void CreateParts(SpreadsheetDocument document, IEnumerable<PropertyViewModel> viewModels)
        {
        ...
        private void BuildWorkSheet(WorksheetPart worksheetPart1, SharedStringTablePart sharedStringTablePart, IEnumerable<PropertyViewModel> viewModels)
        {
            ...
            var comparables = properties.Where(x => x.SaleStatus == PropertyViewModel.PropertySaleStatus.Listed).ToList();

            // create primary header
            BuildHeaderPrimary(sheetData1, sharedStringTablePart);

            // comparables for current year
            IEnumerable<PropertyViewModel> propertiesYtd =
            ...
            List<PropertyViewModel> propertiesPriorYear =
            ...
            foreach (var date in previousYearsDates)
            {
                ...
                IEnumerable<PropertyViewModel> propertiesPreviousYears =
                    comparables.Where(
            ...
            // build Listings
            var listings = properties.Where(x => x.SaleStatus == PropertyViewModel.PropertySaleStatus.Listed).ToList();
        ...
        private MergeCells BuildListings(SheetData sheetData1, SharedStringTablePart sharedStringTablePart, List<PropertyViewModel> listings)
        {
            if (listings.Count() > 0)
            {
        ...
        //    get 
        //    { 
        //        return new[] { "A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N", "O", "P", "Q", "R" }; 
        //    } 
        //}

        private MergeCells BuildProperties(SheetData sheetData1, SharedStringTablePart sstb, IEnumerable<PropertyViewModel> viewModels)
        {
            #region rows
            ...
            foreach (PropertyViewModel viewModel in viewModels)
            {
                ...
                cell6.Append(new CellFormula { Text = String.Format("H{0}/E{0}", rowIndex) });
                ...
                cell11.Append(new CellValue { Text = (viewModel.Occupancy/100M).ToString() });
                ...
                cell15.Append(new CellFormula { Text = String.Format("M{0}/H{0}", rowIndex) });

                var cell17 = new Cell { CellReference = "Q" + rowIndex, StyleIndex = 80U, DataType = CellValues.SharedString };
                cell17.Append(new CellValue { Text = sstb.SharedString("Listing: approximately 7 months. Escrow: N/A. Financing: none recorded. Prior sales: none in prior 3 years. FDIC took over this property that was previously occupied and owned by Riverside Bank. The building is a three-story Class B office building in average condition. The third floor of the building was being leased to a law firm and the remainder of the building was vacant at the time of this sale. ") });
                ...
                cell20.Append(new CellValue { Text = sstb.SharedString(viewModel.Address.AddressLine2) });

                var cell23 = new Cell { CellReference = "E" + rowIndex, StyleIndex = 13U };
                cell23.Append(new CellFormula { Text = String.Format("E{0}/43560", rowIndex - 1) });
                ...
                var cell29 = new Cell { CellReference = "K" + rowIndex, StyleIndex = 20U, DataType = CellValues.SharedString };
                cell29.Append(new CellValue { Text = sstb.SharedString(noiSf) });

                Cell cell31;
                if (viewModel.SaleStatus == PropertyViewModel.PropertySaleStatus.Listed)
                {
                ...
                cell41.Append(new CellValue { Text = sstb.SharedString(viewModel.Parking.ToString()) });
                ...
                cell44.Append(new CellValue { Text = sstb.SharedString("Office") });
                var cell46 = new Cell { CellReference = "K" + rowIndex, StyleIndex = 23U, DataType = CellValues.SharedString };
                cell46.Append(new CellValue { Text = sstb.SharedString(viewModel.OverallRate.ToString()) });
                ...
                cell55.Append(new CellValue { Text = sstb.SharedString("68-43-42-09-01-072-0040") });
